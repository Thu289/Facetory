...
- `diff_gamma` sharpens the response to colour deviations. Sweep tests from 1.0–1.6 showed that `1.35` exposed contour/blush detail without bleeding; values above `1.4` caused banding, while values below `1.2` flattened the makeup contrast. Setting `gamma=1` reverts to a linear mapping, and values below `1` intentionally soften transitions for minimal looks.
- `base_alpha_floor` captures the insight that completely removing the base layer often produces plastic-looking skin. Keeping at least `35 %` of the original alpha preserves microtexture (pores, peach fuzz) even when the colour matches the dominant tone; `0.25` or lower erased pore detail, whereas `0.45+` left too much base tone unattenuated.
- `base_alpha_scale` controls how much of the colour-distance map is blended back in; together with the floor it ensures the total contribution never exceeds the original overlay. A default of `0.65` balances natural transparency and colour pop—`0.5` left overlays too faint, while `0.8` made darker regions feel heavy.
- `blend_mode="softlight"` and `softlight_strength` replicate the perceptual behaviour of soft-light compositing without moving the entire pipeline into the non-linear blend at this stage. A strength of `0.4` keeps highlights gentle while deepening shadow accents; switching to `blend_mode="linear"` disables the remap for debugging parity.
- `highlight_percentile`, `min_highlight_luminance`, `highlight_scale`, and `highlight_blur_sigma` collectively tackle specular glare. Trimming the top `2.5 %` of luminance isolates genuine highlights—lowering to `95th` percentile erased soft sheen, while raising to `99th` missed strong hotspots. Clamping luminance at `0.85` stops noisy dark pixels from being flagged. Scaling highlights to `35 %` preserves a subtle glow (`0.2` looked overly matte, `0.5` still glared), and a Gaussian sigma of `2.5` px smooths masks without leaking into neighbouring regions (smaller sigmas caused hard halos; larger than `3` blurred into non-highlight areas).
