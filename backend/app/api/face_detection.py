...
async def extract_makeup(file: UploadFile = File(...)):
    """
    Extract makeup attributes (lips and eyes color) from a cropped face image using MediaPipe Face Mesh
    """
    # Validate file type
    if not file.content_type.startswith("image/"):
        raise HTTPException(status_code=400, detail="File must be an image.")
    ...
    try:
        ...
        with mp_face_mesh.FaceMesh(static_image_mode=True, max_num_faces=1, refine_landmarks=True) as face_mesh:
            ...
            # Define indices for lips and eyes (using MediaPipe Face Mesh indices)
            ...
            def region_mask(indices):
                ...
                cv2 = __import__('cv2')  # Lazy import to avoid error if not installed
